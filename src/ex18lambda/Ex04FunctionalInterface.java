...
/*
함수형 인터페이스
	: 람다식을 구현하기 위해서는 먼저 인터페이스를 만들고 람다식을 구현할
	추상메서드를 선언한다. 이를 람다식을 만들기 위한 함수형 인터페이스라고 한다.
	※ 함수현 인터페이스에는 오직 하나의 추상메서드만 선언할 수 있다.
@FunctionalInterface
...
//	void studyLambda2(String str); //에러발생됨
}

public class Ex04FunctionalInterface {

	/*
	앞서 정의한 람다식의 참조변수는 IKosmo4 타입이므로 아래와같이 매개변수로
	...
	*/
	...
	public static void main(String[] args) {
		...
		추상메서드를 통해 매개변수의 타입을 유추할수 있어 아래와 같이 생략되었다.
		*/
		IKosmo4 kosmo = (str) ->
			System.out.println("IKosmo4를 구현받아 정의한 람다입니다."+ str);
		/*
		앞에서 정의한 람다식을 메서드의 인수로 전달한다.
		이때 람다식 자체가 전달되는것이 아니라 참조값만 전달된다. 참조값을
