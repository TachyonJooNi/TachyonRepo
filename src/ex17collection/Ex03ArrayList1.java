...
//		Vector<String> list = new Vector<String>();
		...
		객체저장 : add()
			...
			add(인덱스, 객체) : 저장시 데이터를 직접 부여한다. 단, 인덱스를
				...
				해당 인덱스에 객체가 이미 저장되어 있다면 끼워넣기가 된다.
		*/
		list.add("소녀시대");
		list.add("빅뱅");
		list.add("트와이스");
...
//		list.add(5, "소방차 ㅋㅋ"); //에러발생(인덱스를 건너뛸수 없다)
		...
		*/
		...
		방법3 : Iterator(반복자) 사용
			...
			- list에 있는 내용을 기반으로 Iterator객체를 생성하고 이 객체의 참조변수를
			통해 접근한다.
		*/
		System.out.println("\n[반복자(Iterator) 사용]");
		...
		*/
		...
		/*
		방법2 : indexOf()를 통해 해당 객체의 인덱스를 찾은 후 삭제한다. 
		*/
		...
		*/
		System.out.println(list.remove("방탄소년단") ?
				"삭제성공(방탄)" : "삭제실패(방탄)");//삭제성공
		System.out.println(list.remove("오마이걸") ?
				"삭제성공(마이걸)" : "삭제실패(마이걸)");//객체가 없으므로 삭제 실패
