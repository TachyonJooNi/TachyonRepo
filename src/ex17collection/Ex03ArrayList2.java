...
public class Ex03ArrayList2 {
	...
	public static void main(String[] args) {
		...
		켈력션 생성시 Student형의 타입매개변수를 사용한다.
		*/
...
//		ArrayList<Student> list2 = new ArrayList<Student>();
		...
		또한 index로 객체에 접근해야 하므로 get()메서드를 사용한다.
		*/
		System.out.println("[출력1-일반for문]");
		for(int i=0 ; i<list2.size() ; i++) {
			//각 인덱스에 저장된 객체의 참조값을 그래도 출력한다.
		...
		*/
		...
		for(Student st : list2) {
			...
			getter를 통해 접근해야 한다.
			검색을 위해 입력한 이름과 Student객체에 저장된 name과 일치하는지
			비교해서 해당 객체를 찾는다.
			*/
		...
		/*
		인스턴스의 인덱스를 통해 삭제하면 객체 자체가 반환된다.
