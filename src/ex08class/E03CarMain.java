...
class Car {
	...
	*/
	void initialize(String model, int year, String name, int age, int energy) {
		...
		carYear = year;
		/*
		또 다른 객체가 멤버변수로 정의되었으므로, 객체를 생성한 후 각각의
		멤버변수를 초기화 해야 한다.
		*/
		owner = new Human();
		owner.name = name;
...
public class E03CarMain {
	...
	public static void main(String[] args) {
		...
		*/
		...
		/*
		세번째 자동차 객체생성 : 매개변수가 있는 초기화 메서드를 사용하여 객체를
			초기화 한다. 이 경우 매개변수를 통해 다양한 값을 전달할 수 있으므로
			여러형태의 객체를 생성할 수 있다.
			또한 객체 초기화를 위한 중복되는 코드를 줄일수 있으므로 효율적이다.
		*/	
		Car car3 = new Car();
