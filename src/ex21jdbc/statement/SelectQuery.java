...
public class SelectQuery extends ConnectDB {
	...
	ResultSet클래스
		...
		오라클 자료형에 상관없이 getString()으로 모두 출력할 수 있다.(출력된 결과는 결국 문자열이라서)
		그래서 getString()이 제일 많이 사용된다.
	*/
	@Override
	void execute() {
		try {
			...
			*/
			...
			쿼리실행시 사용하는 메서드
			executeUpdate()
				: 쿼리문이 insert, update, delete와 같이 기존 레코드에 변화가 생기는경우
				사용한다. 실행후 영향을 받은 행의 갯수를 int형으로 반환한다.
			executeQuery()
			...
			*/
			...
			while(rs.next()) {
				...
				*/
				...
				0000-00-00 00:00:00형식이므로 적당한 크기로 자르고 싶다면 substring()
				메서드를 사용해야 한다.
				*/
