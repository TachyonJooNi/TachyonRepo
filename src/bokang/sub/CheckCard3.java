...
public class CheckCard3 {
	...
	2.호출시기와 방법 : 객체를 생성할때 딱 한번만 호출된다. 우리가 원할때
		호출할 수 없다.
	3.멤버상수 초기화 : 생성자에서는 멤버상수를 초기화 할수있다. 하지만
		멤버메서드에서는 초기화 할 수 없다.
	*/
	//기본생성자 : 초기값이 없는 빈 객체를 만들수 있다.
	public CheckCard3() {}
	...
	*/
	...
	public void charge(int amount) {
		/*
		충전된 10000원 단위로만 가능하므로 나눠서 나머지가 0인 경우에만
		실행한다.
		*/
		if(amount%10000==0) {
			this.balance += amount;
			...
			System.out.println("1만원 단위로만 충전이 가능합니다.");
		}
	}
	...
	public void payment(int amount) {
		/*
		결제한 금액이 잔고와 동일하거나 적어야 한다.
		따라서 잔고가 결제금액보다는 크거나 같아야 한다.
		*/
		if(this.balance>=amount) {
			...
			System.out.println("잔액이 부족하여 결제하실수 없습니다.");
		}
	}
	...
	public void savingPoint(int amount) {
		...
		*/
		...
		if(amount>100000) {
			...
			System.out.println("10만원 초과사용으로 0.3% 적립해택이 적용되었습니다");
		}
		else {
			plusPoint = amount * 0.1;
			System.out.println("0.1% 적립해택이 적용되었습니다");
		}
		/*
		...
		*/
		...
		/*
		포인트는 보통 원단위 이하는 절삭하므로 아래와 같이 복합대입연사자를 사용하면
