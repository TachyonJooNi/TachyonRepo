...
인터페이스에 
	변수를 선언하면 => public static final이 무조건 추가된다.
		객체생성없이 즉시 사용하는 있는 인터페이스형 상수가 된다. 
...
class Hamburger implements IFoods {
	...
	별도의 메서드없이 객체자체를 출력할 수 있다. 그러므로 필수사항은 아니지만
	오버라이딩 하는것이 좋다. 
	*/ 
	@Override
...
class NormalOrder {
	...
	*/
	...
	public int payment() {
		...
		컬렉션에 저장된 객체타입을 좌측항에 기술한 후 저장된 갯수만큼 순차적으로 꺼낸후
		참조값을 반환한다. 
		 */
