...
규칙
	-메서드는 반드시 class안에 정의해야한다.(자바언어의 특성)
	-메서드안에 또 다른 메서드를 정의할 수 없다.
	-반환타입이 없다면 반드시 void를 명시해야한다.
	-이름의 규칙(Naming rule)은 변수명과 동일하다. (예 : addCalculate())
...
Java에서의 NamingRule(이름규칙)
	...
	-패키지명 : sumple.func => 전체를 소문자로 작성하고 연결단어는 .(닷)으로 표현
...
//		return; //이놈 때문에 아래문장 에러발생(아래코드들이 잉여가 된다)
		
		/*
		위와 같이 메서드 중간에 return을 기술하면 함수가 즉시 종료되므로
		그 아래 코드는 실행되지 않는 쓰레기코드가 된다.
		따라서 return을 사용할때는 반드시 조건문과 함께 써야한다.
		단, 메서드의 마지막 라인에 있다면 조건문은 없어도 된다.
		*/
		if(returnValue%2==0) {
