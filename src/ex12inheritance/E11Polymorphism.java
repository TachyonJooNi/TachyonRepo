...
다형성(Polymorphism)
	...
	2.이런 형태를 동질화(Homogeneous : 호모지니어스)라도 한다.
*/
//부모클래스
class Parent {
	int parentMember;
	void parentMethod() {
		System.out.println("부모의메서드 : parentMethod()");
	}
}
//자식클래스
class Child extends Parent {
	...
	//확장한 멤버메서드
	void childMethod() {
		System.out.println("자식의메서드 : childMethod()");
	}
...
//		parent1.childMember = 1; //에러발생. 부모타입으로 자식멤버에 접근할 수 없다.
		...
		잘못된방법
			-> (Child) parent2.childMember : parent2는 Parent타입이므로
									...
									아래와 같이 Child타입으로 먼저 형변환후 자식의
									멤버변수에 접근해야 한다.
		*/
		((Child)parent2).childMember = 1;
//		(Child)parent2.childMember = 1; //에러발생. 우선순위 생각해서 작성해야한다.
		...
		또학 Object클래스에 정의된 모든 메서드를 별도의 선언없이 사용할 수 있다.
		*/
