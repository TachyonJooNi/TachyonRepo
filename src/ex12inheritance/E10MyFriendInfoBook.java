...
class FriendInfoHandler {
	...
	*/
	...
	/*
	객체배열의 인덱스로 사용할 변수로 초기값은 0으로 설정된다.
	한명의 정보를 추가한 후 ++를 통해 1증가한다. 차후 해당 변수를 통해
	...
	*/
	...
	public FriendInfoHandler(int num) {
		...
		객체배열은 null로 초기화된다. 만약 정수타입의 배열이라면 0으로 초기화된다.
		 */
	...
	public void addFriend(int choice) {
		...
		*/
		...
		3.numOfFriends를 1 증가 시킨다. 후위증가이므로 배열에 먼저 저장된 후
		증가된다.
		*/
		if(choice==1) {
			...
			//객체생성후 참조변수에 저장한 후 객체배열에 추가한다.(2줄로 표현, 객체 타입이 부모타입으로 자동형변환된다)
			HighFriend high = new HighFriend(iName, iPhone, iAddr, iNickname);
			myFriends[numOfFriends++] = high;
			
//			myFriends[numOfFriends] = high;
//			numOfFriends++; //굳이 이렇게 2줄로 쓸 이유가 없다.
//			numOfFriends += 3; //만약 1씩 증가시킬려는게 아니라면 이렇게 쓸 수 있다.
		}
		else if(choice==2) {
			//대딩친구인 경우 전공을 추가로 입력한다.
			System.out.println("전공:"); iMajor = scan.nextLine();
			
			//객체생성과 동시에 객체배열에 추가한다.(1줄로 표현, 객체 타입이 부모타입으로 자동형변환된다)
			myFriends[numOfFriends++] = new UnivFriend(iName, iPhone, iAddr, iMajor);
			/*
			각 객체를 생성 및 초기화한후 객체배열에 추가하면 Friend타입으로
			자동형변환(업캐스팅)이 된다. 부모타입의 참조변수로 자식객체를 참조할수
	...
	public void deleteInfo() {
		...
		*/
		...
		if(deleteIndex==-1) {
			...
			/*
			객체배열에 검색된 원소를 삭제한 후 입력된 위치의 바로 뒤
			원소를 앞으로 하나씩 복사한다.
