...
class ShowThread extends Thread{
	...
	Thread클래스의 run() 메서드는 오버라이딩 한것으로 해당 메서드는 직접
	호출하면 안되고, start() 메서드를 통해 간접적으로 호출해야한다.
	만약 직접 호출하면 단순한 실행만 될뿐 쓰레드가 생성되지 않는다.ㄴ
	*/
	@Override
	public void run() {
		for(int i=1; i<=100; i++) {
			...
			/*
			Tread클래스의 static메서드로 실행의 흐름을 주어진 시간만큼
			block상태로 전환시켜 준다.
			1000분의 1초 단위로 기술하게 되므로 아래는 0.01 초만큼 block
			시키게된다.
			*/
			try {
				sleep(10);
